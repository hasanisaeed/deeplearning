...
def generate_lstmfcn(MAX_SEQUENCE_LENGTH, NB_CLASS, NUM_CELLS=8):
    # Run | Block #1 | Block #2 | Block #3 | Result |
    # --------------------------------------------------
    #  #1 |    256   |    256   |    64    | 0.806742  |
    # --------------------------------------------------
    #  #2 |    256   |    128   |    64    |    0.65   |-> Epoch: 500
    # --------------------------------------------------
    #  #3 |    256   |    128   |    128   |    0.61   |-> Epoch: 500
    # --------------------------------------------------
    ...
    y = Permute((2, 1))(ip)
    y = Conv1D(_block1, 8, padding='same', kernel_initializer='he_uniform')(y)
    y = BatchNormalization()(y)
    y = Activation('relu')(y)

    y = Conv1D(_block2, 5, padding='same', kernel_initializer='he_uniform')(y)
    y = BatchNormalization()(y)
    y = Activation('relu')(y)

    y = Conv1D(_block3, 3, padding='same', kernel_initializer='he_uniform')(y)
    y = BatchNormalization()(y)
    y = Activation('relu')(y)
